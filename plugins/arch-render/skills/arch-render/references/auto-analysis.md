...
| **Bare upload, no instruction** — an image or architectural source with no stated task | After environment selection, generate the relevant text-only pack from `automatic-prompt-pack.md`. Then offer **§86 Smart Image Action Menu** only for further analysis or execution. Do not spend credits. |

---

# 86 · Smart Image Action Menu

Use only after the automatic Prompt Pack on a bare upload, or when the user explicitly asks for available
actions. Run the **silent read** (Step 1) first so the menu reflects the source.
