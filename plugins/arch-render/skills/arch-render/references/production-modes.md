...
**Reject the output** (do not present it as successful) if the render shows:
...
**Corrective protocol:** explain the defect briefly, simplify the prompt, and prepare a corrected
request. Execute another paid attempt only when it was included in the approved attempt budget or the
user explicitly approves it. Never burn API calls in a loop or present a broken render as successful.
...
- Run **§83** on every output: `Read` it, check it, reject anything that drifts from the
